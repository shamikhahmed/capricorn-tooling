...
Current step: **Step R** — real LH on Soul/Pulse stepR (thresholds unmet); C-57 mostly live; **ARCH-02 done** → ARCH-03 adapters; kill-list continues
...
- **ARCH-02 ✅** static viewer `shared/architecture/viewer/{index.html,viewer.js,viewer.css}` + fixture demo data + `npm run architecture:viewer` / `architecture:viewer:stress`; `analyze` copies viewer assets into `--out`
